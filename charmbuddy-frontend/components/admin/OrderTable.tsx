...
type OrderTableProps = {
  ...
  onFinish: (order: AdminOrder) => void;
};

function StatusBadge({ status }: { status: string }) {
  const colorClass =
    status === "Finished" ? "bg-purple-100 text-purple-700" :
...
function ActionButton({ label, onClick, tone = "dark" }: { label: string; onClick: () => void; tone?: "dark" | "red" | "blue" }) {
  ...
  return (
    <motion.button
      className={`rounded-[10px] px-[10px] py-[6px] font-satoshi text-[12px] tracking-[0.8px] ${toneClass}`}
      onClick={onClick}
      type="button"
...
export default function OrderTable({ orders, isLoading, onShip, onFinish }: OrderTableProps) {
  if (isLoading) {
  ...
  if (orders.length === 0) {
    return (
      <div className="rounded-[20px] border border-black/10 bg-white/75 p-[16px]">
        <p className="font-satoshi text-[16px] text-black/70">Belum ada order.</p>
      </div>
    );
  }
        ...
        <table className="w-full min-w-[900px] border-collapse">
          <thead>
            <tr className="border-b border-black/10 bg-[#8798ff]/20 text-left">
              ...
              <th className="px-[12px] py-[10px] font-satoshi text-[13px] tracking-[1px]">Total</th>
              <th className="px-[12px] py-[10px] font-satoshi text-[13px] tracking-[1px]">Status</th>
              <th className="px-[12px] py-[10px] font-satoshi text-[13px] tracking-[1px]">Tracking</th>
          ...
          <tbody>
            {orders.map((order) => (
              <tr className="border-b border-black/5" key={order.id}>
                ...
                <td className="px-[12px] py-[10px] font-satoshi text-[14px]">{order.user?.name ?? "-"}</td>
                ...
                <td className="px-[12px] py-[10px]">
                  <div className="flex flex-wrap gap-[6px]">
                    ...
                    {order.status === "Shipped" && (
                      <ActionButton label="Finish" onClick={() => onFinish(order)} tone="blue" />
                    )}
                    {(order.status === "Finished") && (
                      <span className="font-satoshi text-[12px] text-black/40">Selesai</span>
                    )}
                    {order.status === "Pending" && (
                      <span className="font-satoshi text-[12px] text-black/40">
                        {order.payment?.status === "Rejected" ? "Pembayaran ditolak" : "Menunggu pembayaran"}
                      </span>
                    )}
                  </div>
                </td>
        ...
        {orders.map((order) => (
          <article className="rounded-[16px] border border-black/10 bg-white/80 p-[12px]" key={order.id}>
            <div className="flex items-center justify-between">
              <p className="font-fanlste text-[22px]">#{order.id}</p>
              <StatusBadge status={order.status} />
            </div>
            <p className="mt-[8px] font-satoshi text-[14px] text-black/70">{order.user?.name ?? "-"}</p>
            ...
            <div className="mt-[10px] flex flex-wrap gap-[6px]">
              ...
              {order.status === "Pending" && (
                <span className="font-satoshi text-[12px] text-black/40">Menunggu pembayaran</span>
              )}
            </div>
          </article>
        ))}
