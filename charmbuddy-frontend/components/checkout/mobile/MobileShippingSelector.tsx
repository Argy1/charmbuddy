  ...
  return (
    <section className="w-full rounded-[20px] border border-white/70 bg-[rgba(255,255,255,0.5)] px-[14px] py-[14px] backdrop-blur-[12px]">
      <div className="mb-[14px] flex items-end justify-between gap-[12px]">
        <div>
          <p className="font-fanlste text-[22px] tracking-[2px] text-black">Shipping</p>
          <p className="mt-[4px] font-satoshi text-[13px] tracking-[1px] text-black/65">Pilih courier dan service yang paling cocok.</p>
        </div>
        {errorMessage ? (
          <button
            className="rounded-[999px] border border-black/30 bg-white px-[10px] py-[4px] font-satoshi text-[12px] font-bold tracking-[1px] text-black"
            onClick={onRetry}
            type="button"
          >
            RETRY
        ...
        <div className="space-y-[12px]">
          {grouped.map((group) => (
            <div className="space-y-[8px]" key={group.courier}>
              <p className="font-satoshi text-[13px] font-black tracking-[2px] text-black/75">{group.courier}</p>
                    ...
                    >
                      <div className="flex items-start justify-between gap-[12px]">
                        <div className="min-w-0">
                          <p className="font-satoshi text-[15px] font-black tracking-[1.5px] text-black">{option.service}</p>
      ...
      {!isLoading && grouped.length === 0 ? (
        <div className="rounded-[14px] border border-black/15 bg-white/70 px-[12px] py-[12px]">
          <p className="font-satoshi text-[13px] tracking-[1px] text-black/65">{errorMessage ?? "Opsi pengiriman belum tersedia."}</p>
        </div>
      ) : null}
    </section>
  );
