...
export default function MobileVoucherCard({
  ...
  return (
    <section className="w-full rounded-[20px] border border-white/70 bg-[rgba(255,255,255,0.5)] px-[14px] py-[14px] backdrop-blur-[12px]">
      <div>
        <p className="font-fanlste text-[22px] tracking-[2px] text-black">Voucher</p>
        <p className="mt-[4px] font-satoshi text-[13px] tracking-[1px] text-black/65">Contoh kode: CHARM10 atau WELCOME20.</p>
      </div>
      ...
      <div className="mt-[12px] rounded-[14px] border border-black/20 bg-white px-[12px] py-[8px]">
        <div className="flex items-center gap-[8px]">
          <input
            ...
            className="min-w-0 flex-1 bg-transparent font-satoshi text-[15px] tracking-[1.5px] text-black outline-none placeholder:text-black/45"
          ...
          />
          <motion.button
            className="rounded-[999px] bg-black px-[10px] py-[6px] font-satoshi text-[12px] font-black tracking-[1px] text-white disabled:opacity-55"
      ...
      {message ? (
        <div className={`mt-[8px] rounded-[10px] px-[10px] py-[8px] font-satoshi text-[12px] tracking-[0.8px] ${state === "error" ? "border border-red-500/20 bg-red-50 text-red-700" : "border border-green-500/20 bg-green-50 text-green-700"}`}>
          <p>{message}</p>
          {state === "error" ? (
            <button className="mt-[4px] font-satoshi text-[11px] font-black tracking-[1px] text-red-700 underline" onClick={onApply} type="button">
              Coba Lagi
            </button>
          ) : null}
        </div>
