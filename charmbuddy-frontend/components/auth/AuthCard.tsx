...
export default function AuthCard({ mode = "login" }: AuthCardProps) {
  ...
  return (
    <motion.form
      className={`backdrop-blur-[6.9px] bg-[rgba(255,255,255,0.15)] border border-[rgba(255,255,255,0.69)] border-solid rounded-[20px] shadow-[0px_4px_4px_0px_rgba(0,0,0,0.25)] w-full max-w-[788px] px-[16px] py-[24px] sm:px-[24px] sm:py-[28px] xl:px-[34px] xl:py-[30px] ${isRegister ? "h-auto xl:min-h-[649px]" : "h-auto xl:min-h-[473px]"
        }`}
    ...
    >
      {!isRegister ? (
        <div>
          <div>
            <Link className="-translate-y-20 inline-block text-white text-2xl" href={routes.home}>
              Home
            </Link>
          </div>
          <div className="mx-auto flex w-full max-w-[695px] flex-col items-center gap-[18px] sm:gap-[24px]">
            <h1 className="w-full text-center font-fanlste text-[clamp(48px,14vw,60px)] leading-[normal] text-white">Login</h1>
            ...
            <motion.label className="bg-white flex h-[60px] w-full items-center px-[12px] py-[14px] sm:px-[18px]">
              <div className="flex w-full min-w-0 items-center gap-[10px]">
                <input
                  className="min-w-0 flex-1 bg-transparent font-satoshi text-[clamp(16px,4vw,22px)] leading-[normal] text-black outline-none"
            ...
            <InteractivePress>
              <button className="bg-white rounded-[10px] h-[48px] w-[120px] px-[16px] py-[8px] text-center disabled:opacity-50 sm:w-[110px]" disabled={isLoading} type="submit">
                <span className="font-satoshi text-[clamp(20px,5vw,24px)] leading-[normal] text-black">Login</span>
              </button>
            </InteractivePress>
            ...
            <div className="flex w-full flex-wrap items-center justify-center gap-x-[8px] gap-y-[2px] text-center text-[clamp(16px,3.4vw,20px)] leading-[normal] text-white">
              <p className="font-satoshi">Dont Have an Account?</p>
              <Link className="font-satoshi font-[700]" href={routes.register}>
                Register
              </Link>
            </div>
          </div>
        ...
        <div>
          ...
          <div className="mx-auto flex w-full max-w-[695px] flex-col items-center gap-[18px] sm:gap-[20px]">
            <h1 className="w-full text-center font-fanlste text-[clamp(48px,14vw,60px)] leading-[normal] text-white">Register</h1>

            <motion.label className="bg-white flex h-[60px] w-full items-center px-[12px] py-[14px] sm:px-[18px]">
              <div className="flex w-full min-w-0 items-center gap-[10px]">
                <input
                  className="min-w-0 flex-1 bg-transparent font-satoshi text-[clamp(18px,5vw,24px)] leading-[normal] text-black outline-none"
            ...
            <InteractivePress>
              <button className="bg-white rounded-[10px] w-[120px] h-[48px] px-[16px] py-[8px] text-center disabled:opacity-50" disabled={isLoading} type="submit">
                <span className="font-satoshi text-[clamp(20px,5vw,24px)] leading-[normal] text-black">Register</span>
              </button>
            </InteractivePress>
          </div>

          <div className="flex w-full flex-wrap items-center justify-center gap-x-[8px] gap-y-[2px] my-5 text-center text-[clamp(16px,3.4vw,20px)] leading-[normal] text-white">
            <p className="font-satoshi">Already Have an Account?</p>
            <Link className="font-satoshi font-[700]" href={routes.login}>
              Login
            </Link>
          </div>
        </div>
      ...
      {error ? <p className="mt-[8px] break-words px-[8px] text-center font-satoshi text-[16px] text-white">{error}</p> : null}
    </motion.form>
  );
}
